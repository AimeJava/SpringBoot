<template>
    <Table :columns="columns10" :data="data">
        <template slot-scope="{ row, index }" slot="action">
            <Button type="primary" size="small" style="margin-right: 5px" @click="show(index)">打印订单</Button>
            <Button type="error" size="small" @click="remove(index)">接单</Button>
        </template>
    </Table>
</template>
<script>
    ...
    export default {
        ...
        created (){
            this.$axios.get("http://localhost:8888//order")
                .then(resp => {
                    this.data = resp.data;
